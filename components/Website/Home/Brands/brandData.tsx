// import { Brand } from "@/types/brand";

const brandData = [
  {
    id: 23,
    title: "Sector",
    submenu: [
      { id: 31, title: "Energy Sector", path: "/energy_sector" },
      { id: 32, title: "Food and Agriculture", path: "/food_and_agriculture" },
      { id: 33, title: "Industrial Sector", path: "/industrial_sector" },
      { id: 34, title: "Automotive Sector", path: "/automotive_Sector" },
      { id: 35, title: "Governance and Policy", path: "/government_and_policy" },
      { id: 36, title: "Technology and Media", path: "/technology_and_media" },
      { id: 37, title: "Health Care Sector", path: "/healthcare_sector" },
    ],
  },
];

export default brandData;



// const brandData: any[] = [
//   {
//     id: 0.25,
//     name: "Government Agencies",
//     href: "#",
//     image: "/images/brand/brand-light-01.svg",
//     imageLight: "/images/brand/brand-dark-01.svg",
//   },
//   {
//     id: 0.3,
//     name: "Oil & Gas Industry",
//     href: "#",
//     image: "/images/brand/brand-light-02.svg",
//     imageLight: "/images/brand/brand-dark-02.svg",
//   },
//   {
//     id: 0.4,
//     name: "Educational Institutions",
//     href: "#",
//     image: "/images/brand/brand-light-03.svg",
//     imageLight: "/images/brand/brand-dark-03.svg",
//   },
//   {
//     id: 0.5,
//     name: "Corporate Organizations",
//     href: "#",
//     image: "/images/brand/brand-light-04.svg",
//     imageLight: "/images/brand/brand-dark-04.svg",
//   },
//   {
//     id: 0.6,
//     name: "Safety Management",
//     href: "#",
//     image: "/images/brand/brand-light-05.svg",
//     imageLight: "/images/brand/brand-dark-05.svg",
//   },
//   {
//     id: 0.7,
//     name: "Environmental Sectors",
//     href: "#",
//     image: "/images/brand/brand-light-06.svg",
//     imageLight: "/images/brand/brand-dark-06.svg",
//   },
// ];
//
// export default brandData;
